...
[<mark>秒杀</mark>](#seckill)：先用令牌桶进行限流，放行之后异步下单，先运行Lua脚本：先判断库存够不够，再判断是否下过单，若未下过单，才能扣减redis库存和sadd成员，判定具备购买资格。生成一个全局id作为订单id，把订单信息发送到MQ，如果失败则回滚redis，前端轮询订单的status。防止订单发送错误，可以lua脚本里面写一条预扣订单记录，再加定时任务扫描补偿。
...
使用令牌桶限流防止突发流量：异步下单，先运行Lua脚本，判断库存够不够，判断是否下过单，若未下过单，才能扣减Redis库存，脚本运行成功，有购买资格。则生成一个全局Id作为订单id，生成订单信息，把订单保存到消息队列,redis回滚。
...
2. 同时将 `order_id` 存入待补偿集合Zset（时间戳，订单id）：

```
ZADD stock:recovery {timestamp} {order_id}
```

> MQ 生产者端发送成功（confirm）则使用lua删除这个预扣记录和待补偿集合里面的元素

- 若 MQ 发送成功，**使用Lua删除预扣记录和待补偿集合的元素**：

```
DEL stock:lock:{order_id}
ZREM stock:recovery {order_id}
```
...
- 检查预扣记录是否存在（`EXISTS stock:lock:{order_id}`）：
  - **存在**：说明 MQ 未发送成功 → **回滚库存**（`INCR` 库存）并清理记录。
  - **不存在**：说明 MQ 已成功 → 删除 Recovery 的元素。
...
2. **补偿机制：最终一致性兜底**。如果重试到达一定次数就变成死信，死信监听器接受消息之后出发redis回滚对库存加1，对集合进行remove的操作。
