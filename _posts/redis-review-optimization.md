...
[<mark>秒杀</mark>](#seckill)：先用令牌桶进行限流，放行之后异步下单，先运行Lua脚本：先判断库存够不够，再判断是否下过单，若未下过单，才能扣减redis库存和sadd成员，判定具备购买资格。生成一个全局id作为订单id，把订单信息发送到MQ，如果失败则回滚redis，前端轮询订单的status。
