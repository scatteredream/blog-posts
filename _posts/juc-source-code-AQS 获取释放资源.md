...
title: 一行一行源码分析清楚AbstractQueuedSynchronizer(转自Javadoop)
date: 2017-10-19
tags: 
- AQS
- 源码
...
static final class FairSync extends Sync {
    ...
    // 我们看到，这个方法，如果tryAcquire(arg) 返回true
    // 否则，acquireQueued方法会将线程压到队列中
    public final void acquire(int arg) { // 此时 arg == 1
        // 首先调用tryAcquire(1)一下，名字上就知道，这个只是试一试：核心是CAS尝试获取，公平锁FairSync会查看队列情况
        // 因为有可能直接就成功了呢，也就不需要进队列排队了（公平锁限制，只有空队列才有这种情况）
    ...
    protected final boolean tryAcquire(int acquires) {
        ...
        if (c == 0) {
      	...
      	// 会进入这个else if分支，说明是重入，需要操作：state=state+1
        // 这里不存在并发问题
        ...
        // 回到上面一个外层调用方法继续看: 也就是 acquireQueued:
        // if (!tryAcquire(arg) 
        //        && acquireQueued(addWaiter(Node.EXCLUSIVE), arg)) 
        //     selfInterrupt();
        return false;
    ...
    private Node addWaiter(Node mode) {
        ...
        // 以下几行代码想把当前node加到链表的最后面去，也就是进到队列的队尾
        Node pred = tail;
